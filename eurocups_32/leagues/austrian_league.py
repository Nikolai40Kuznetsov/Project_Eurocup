...
import pandas as pd
austrian_league = {
    'Клубы': ["Штурм", "Аустрия", "Ред Булл Зальцбург", "Вольфсберг",
              "Рапид Вена", "БВ Линц", "Хартберг", "Рид",
              "Райндорф Альтах", "Грацер", "ЛАСК", "Тироль"],
    'Евроочки': ["26000", "9500", "43000", "5000",
                 "28250", "0", "0", "0", 
                 "0", "0", "21000", "0"], # сила лиги 11000
    'Очки силы': [9, 6, 10, 6,
                  7, 5, 4, 2,
                  3, 5, 8, 4],
...
games.play_league(4, 12, austrian_league)
df = pd.DataFrame(austrian_league)
df = df.sort_values(
...
df['Место'] = df.index + 1
# print(df)
cup_winner = games.play_cup(12, austrian_league)
def champions_qual():
