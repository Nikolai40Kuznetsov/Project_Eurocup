...
df['Место'] = df.index + 1
# print(df)
cup_winner = games.play_cup(12, finnish_league, "Финляндия")
def champions_qual():
    return df['Клубы'][0], df["Евроочки"][0], "Финляндия"
def europe_qual():
    return df['Клубы'][1], df["Евроочки"][1], "Финляндия"
def conference_qual():
    return df['Клубы'][2], df["Евроочки"][2], "Финляндия"